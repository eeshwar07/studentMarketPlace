...
export default function ForgotPassword() {
    ...
    const sendEmail = () => {
        emailjs.sendForm('service_t1u2k7d', 'template_ewk0zyj', form.current, '5n7aiK9R8erkUwcvW')
          .then((result) => {
              alert(result.text);
    ...
    const handleSubmit = (e) => {
      ...
      if(random === parseInt(otp)){
        Axios.get(baseURL+`forgotpassword/${email}`).then(function (
          response
        ) {
          console.log(response.data);
          // console.log(users)
          if (response.data)
              alert("The password is:"+response.data[0].password);
              navigate("/login.html");
        });
      }
      else{
...
<div style={{alignItems:'center',display:'flex',justifyContent:'center',marginTop:'20px'}}>
    <form id="login" ref={form} onSubmit={(e) =>generateRandomNumber(e)}>
        <center>
          <div> 
              <div><label htmlFor="email"><b>Enter E-Mail id:</b></label></div><br></br>
              <div><input type="email"  onChange={(e)=>setEmail(e.target.value)} value={email} name="email" required/></div><br></br>
          </div>

          <input type="hidden" name="random" value={random}></input>
          <button id="logbutton">Submit</button><br></br><br></br>
        </center>
      </form>
</div>
...
<div style={{alignItems:'center',display:'flex',justifyContent:'center',marginTop:'20px'}}>
    <form id="login" onSubmit={handleSubmit}>
        <center>
          <div> 
              <div><label htmlFor="otp"><b>Enter OTP:</b></label></div><br></br>
              <div><input type="text" onChange={(e)=>setOtp(e.target.value)} name="otp" value={otp} required/></div><br></br>
          </div>
          <button id="logbutton">Submit</button><br></br><br></br>
        </center>
      </form>
