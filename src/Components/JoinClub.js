...
export default function JoinClub() {
    ...
    function getData() {
        Axios.get(baseURL+"joinclub").then(function (
          response
        ) {
      ...
      const addClubCart = (user) =>{
        ...
        Axios.post(baseURL+`clubcart/${user.id}/${stud_id}`,
            user
    ...
    <>
      ...
      <div  style={{display: 'flex'}}>
          ...
          <h3 style={{marginLeft:'20px'}}>
              <a href="home.html" style={{fontSize: '100%', color:'white'}}>STUDENT<br></br>MARKETPLACE</a></h3>
          </div>
...
<>    
  <div className="card"  key={key} style={{marginLeft:'20px'}}>
            ...
            <div><button onClick={() =>addClubCart(user)} className="c-btn">JOIN!!!</button></div>
        </div>
  </>
  ))}
  </div>
